...
export default function ProductClientPage({ initialProducts, slug }: { initialProducts: Product[], slug: string }) {
    ...
    const initialProd = findCncBySlug(initialProducts, slug) || initialProducts[0]
    ...
    useEffect(() => {
        if (products.length > 0) {
            let idx = 0;
            if (slug) {
                const found = findCncBySlug(products, slug)
                if (found) {
    ...
    const selectProduct = (idx: number) => {
        ...
        if (selectedProd) {
            router.push(`/dau-cnc/san-pham/${toSlug(selectedProd.name, selectedProd.id)}`, { scroll: false })
            setActiveIndex(idx)
